...
def handle_postback(event):
    ...
    if(user.function_status == 'quiz'):
        ...
        if(user.isChangingLevel_Q==True):
            ...
            user.score = int(user_data[data_row][2])
            user.qa_score = int(user_data[data_row][3])
            user_sheet.update_cell(user_index, 3, user.score + user.star_num_Q)
            user_sheet.update_cell(user_index, 4, user.qa_score + user.star_num_Q)
...
#------------------聽力回饋判斷-------------------
        ...
        if(user.isChangingLevel_L==True):
            ...
            user.score = int(user_data[data_row][2])
            user.lis_score = int(user_data[data_row][4])
            user_sheet.update_cell(user_index, 3, user.score + user.star_num_L)
            user_sheet.update_cell(user_index, 5, user.lis_score + user.star_num_L)
...
#-----------------語音回饋判斷----------------------------------------
        ...
        if(event.postback.data == 'L' or event.postback.data == 'M' or event.postback.data == 'H'):
            ...
            user.score = int(user_data[data_row][2])
            user.speech_score = int(user_data[data_row][5])
            user_sheet.update_cell(user_index, 3, user.score + user.star_num_s)
            user_sheet.update_cell(user_index, 6, user.speech_score + user.star_num_s)
